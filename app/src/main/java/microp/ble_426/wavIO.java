...
*/
    ...
    public wavIO(String tmpPath, byte[] tmpData) {
        myPath = tmpPath;
        myData = tmpData;
    }
    ...
    public static byte[] wavGet(byte[] audio) {
        byte[] wav;
        byte[][] storage = new byte[17][];

        storage[0] = ("RIFF").getBytes();
        storage[1] = intToByteArray( 36+audio.length);//whole size
        storage[2] = ("WAVE").getBytes();
        storage[3] = ("fmt ").getBytes();
        storage[4] = intToByteArray(16);
        storage[5] = shortToByteArray((short) 1);
        storage[6] = shortToByteArray((short) 1);
        storage[7] = intToByteArray((int) 10000);
        storage[8] = intToByteArray((int) 10000);
        storage[9] = shortToByteArray((short) 1);
        storage[10] = shortToByteArray((short) 8);
        storage[11] = ("data").getBytes();
        storage[12] = intToByteArray(audio.length);
        storage[13] = audio;// 44 - the actual data itself - just a long string of numbers
        wav = new byte[storage[0].length + storage[1].length];

        System.arraycopy(storage[0], 0, wav, 0, storage[0].length);
        System.arraycopy(storage[1], 0, wav, storage[0].length, storage[1].length);
        System.out.println("wav"+wav);
        for (int i = 2; i < 14;i++){
            System.out.println("storage"+storage[i]);
            byte[] c = new byte[wav.length + storage[i].length];
            System.arraycopy(wav, 0, c, 0, wav.length);
            System.arraycopy(storage[i], 0, c, wav.length, storage[i].length);
            wav = c;
        }
        return wav;
    }
